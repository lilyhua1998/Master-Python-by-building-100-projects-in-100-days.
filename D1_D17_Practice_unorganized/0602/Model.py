...
class AAE():
    def __init__(self, Z, n_features, BATCH_SIZE, GANorWGAN, nodes, n_dis, n_decoder):
        ...
        self.optimizer = tf.keras.optimizers.Adam(5e-7)  # Nadam
        if GANorWGAN == 'WGAN':
        ...
        self.decoder = build_decoder(Z, self.nodes, n_features)
        ...
        self.adversarial_autoencoder = Model(inputs=X, outputs=[reconstructed_X, validity])
        self.adversarial_autoencoder.compile(loss=['mse', self.loss], loss_weights=[0.01, 0.09],
                                             optimizer=self.optimizer)
        # self.generator = Model(inputs=X, outputs=encoded_repr)
        self.autoencoder = Model(inputs=X, outputs=reconstructed_X)
        self.autoencoder.compile(loss='mse', optimizer=self.optimizer)
        # decoder
        self.endis = Model(inputs=X, outputs=validity)
        self.endis.compile(loss=self.loss, optimizer=self.optimizer)
        #
        self.decoder = build_decoder(Z, nodes, n_features)
        self.decoder.compile(loss='mse', loss_weights=[0.001], optimizer=self.optimizer)

        # saving the events of the generator and critic
        adversarial_autoencoder_log_dir = './content/adversarial_autoencoder'
        discriminator_log_dir = './content/discriminator'
        self.adversarial_autoencoder_summary_writer = tf.summary.create_file_writer(adversarial_autoencoder_log_dir)
    ...
    def train(self, Z, BATCH_SIZE, dataset, epochs, scaler, scaled, X_train_scaled):
        ...
        for epoch in range(epochs):
            ...
            # print("[Dis(w): %f], [endis(w)]: %f, [decoder(mse)]: %f],[autoencoder(mse)]: %f]" % (self.c1_hist[epoch], self.endis_hist[epoch], self.decoder_hist[epoch], self.autoencoder_hist[epoch]))

            print("[Dis(w): %f], [endis(w)]: %f, [decoder(mse)]: %f]" % (
            self.c1_hist[epoch], self.endis_hist[epoch], self.decoder_hist[epoch]))
