...
from keras.layers import (Dense, Reshape, Flatten, LeakyReLU, ReLU, ELU, LayerNormalization, Softmax, Dropout,
                          BatchNormalization)
...
def build_encoder(Z, nodes, n_features):
    ...
    latent_repr = Lambda(lambda p: p[0] + backend.random_normal(backend.shape(p[0])) * backend.exp(p[1] / 2))(
        [mu, sigma])
    # lamda=Z=2/2*2
